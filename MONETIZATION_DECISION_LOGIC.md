...
| 技术栈 | Next.js 16 + Netlify + Neon PostgreSQL | 已有完整架构 |
...
**决策原则 13：每次部署自动同步数据**
- `netlify.toml` 的构建命令：`npx prisma generate && node seed_db3.js && npm run build`
...
| 层级 | 位置 | 示例 | 说明 |
|------|------|------|------|
| 文件（Git 提交） | `.env.production` | `NEXT_PUBLIC_ADSENSE_PUB_ID` | 公开或半公开变量 |
| 平台（Netlify 后台） | Environment Variables | `DATABASE_URL` | 敏感信息不提交到 Git |
...
|----|--------|----------|-----------|
| 对比页面生成 | 高 | 无 | 2-3 天 |
| Best X Tools 页面 | 高 | 无 | 1-2 天 |
| CTA 按钮优化 | 中 | 联盟链接填充完成 | 1 天 |
| Google Analytics 4 | 中 | 无 | 0.5 天 |
| 联盟链接填充 | 高 | 各平台审核通过 | 持续 |
| Newsletter 对接 | 低 | 无 | 0.5 天 |
| 社媒运营 | 中 | 无 | 持续 |
...
| `netlify.toml` | 部署配置 | 决策原则 13 |
| `.env.production` | 生产环境变量 | 决策原则 15 |
