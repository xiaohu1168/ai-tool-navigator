    ...
    <div className='min-h-screen flex flex-col'>
      ...
      <main className='flex-1 max-w-4xl mx-auto px-3 md:px-4 py-5 md:py-8 w-full'>
        ...
        <div className='flex flex-wrap gap-3 mb-6 md:mb-8'>
          <a href={tool.url} target='_blank' rel='noopener noreferrer' className='bg-primary text-white px-4 md:px-6 py-2.5 rounded-lg font-medium hover:bg-primary/90 flex items-center gap-2 text-sm transition-colors'>
            Visit Official Site <ExternalLink className='w-4 h-4' />
          </a>
          {affiliateLinks.length > 0 ? affiliateLinks.map(link => (
            <a key={link.id} href={'/api/track/affiliate?linkId=' + link.id} target='_blank' rel='noopener noreferrer' className='bg-background border-2 border-primary text-primary px-4 py-2.5 rounded-lg font-medium hover:bg-primary/5 flex items-center gap-2 text-sm transition-colors'>
              {link.label} <ExternalLink className='w-3.5 h-3.5' />
            </a>
          )) : (
            <a href={tool.url} target='_blank' rel='noopener noreferrer' className='bg-green-600 text-white px-4 md:px-6 py-2.5 rounded-lg font-medium hover:bg-green-700 flex items-center gap-2 text-sm transition-colors'>
