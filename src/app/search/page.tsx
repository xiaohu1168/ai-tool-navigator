...
async function SearchResults({ query }: { query: string }) {
  const results = await searchTools(query);
    ...
    return (
      <div className="text-center py-16">
        <h2 className="text-xl font-semibold text-foreground mb-2">Search AI Tools</h2>
  ...
  return (
    <div>
      <p className="text-sm text-muted-foreground mb-4">{results.length} tool{results.length !== 1 ? "s" : ""} found for &quot;{query}&quot;</p>
